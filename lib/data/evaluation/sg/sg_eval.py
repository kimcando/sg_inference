...
def evaluate(gt_classes, gt_boxes, gt_rels,
    ...
    # HERE need to extract
    # pred_triplets, relation_scores
    # breakpoint()
...
def _triplet(predicates, relations, classes, boxes,
    ...
    triplet_sub_pred_obj_scores = np.zeros([num_relations,3]).astype(np.float32)
    triplet_dict = dict()
    triplet_dict['sub'] = []
    triplet_dict['obj'] = []
    for i in range(num_relations):
        ...
        # score value here
        triplet_sub_pred_obj_scores[i, 0] = class_scores[sub_i]
        triplet_sub_pred_obj_scores[i, 2] = class_scores[obj_i]
        triplet_sub_pred_obj_scores[i, 1] = predicate_scores[i]

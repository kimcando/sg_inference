...
class SceneGraphGeneration:
    ...
    def visualize_detection(self, dataset, img_ids, imgs, predictions,live=False,visualize_folder="visualize",
        ...
        for i, prediction in enumerate(predictions):
            ...
            if live:
                cv2.imshow('raw_image', result); cv2.waitKey(10)
            else:
                cv2.imwrite(os.path.join(visualize_folder + '/' + raw_folder, "raw_detection_{}.jpg".format(img_ids[i])),
                            result)
                img = cv2.imread(f'/home/ncl/ADD_sy/inference/sg_inference/visualize/raw/raw_detection_{img_ids[0]}.jpg')
            ...
            if live:
                cv2.imshow('bounding_box_image', result); cv2.waitKey(10)
            else:
